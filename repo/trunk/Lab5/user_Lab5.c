...
float u =0.0;
float Kprop = 0.1;
int mainclocktimer = 0;

void mainclockfunc(void){
    mainclocktimer++;
    if(mainclocktimer == 100){
        UART_printfLine(1, "A %0.2f U %0.2f R %0.2f \n\r", b0voltage, u, reference);
        mainclocktimer = 0;
    }
}

//void initiateADC(void){
//
//    AdcbRegs.ADCSOCPRICTL.bit.RRPOINTER = 0x10;  //rr pionter reset, soc0 next
//    AdcbRegs.ADCSOCFRC1.all |= 0x3;
//
//}
...
void ADChwifunc(void)
{
    ...
    b0voltage = 10 - ((20.0*adcb0result)/4095.0);
    //b1voltage = 10 - ((20.0*adcb0result)/4095.0);

    //Input
    u = (reference - b0voltage)*Kprop;
