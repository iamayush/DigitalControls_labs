...
// SWIs
extern const Swi_Handle Swi_loaded;
extern const Swi_Handle Swi_count;
...
//Switch Read
int Switch1State = 0; // reads switch states here
int Switch2State = 0;
int Switch3State = 0;
int Switch4State = 0;
int SwitchState=0;
...
void timeloadfunc(void){
    timeload(10);
}

void timecountfunc (void){
    myms++;
    mytime+=0.001;

    if ((myms%200) == 0){
        //UART_printfLine(1,"t_f=%.4f",mytime);
        UART_printfLine(2,"t_int=%.4f",(myms*0.001));
    }
    ...
    if((myms%100)==0){
        ...
        if(SwitchState!=PrevSwitchState){
            ...
            Semaphore_post(Semaphore_switchesChanged);
        }
    }
}
...
void mytaskfunc(void){
    while(1){
        ...
        switch(SwitchState){
        case 0 :
            UART_printfLine(1,"%d", SwitchState);
            break;
        case 1 :
                    UART_printfLine(1,"%d", SwitchState);
                    break;
        ...
        default : UART_printfLine(1,"Error");
        }
    }
}
