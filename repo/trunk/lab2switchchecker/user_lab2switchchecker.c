...
// Clock Function called every 1s
void doOneSecond(void){
    switchstate = ReadSwitches();
    UART_printfLine(1,"Switch %d", switchstate);
    SetLEDsOnOff(switchstate);
}
...
void SetLEDsOnOff(int leds){
    ...
    if ((leds & 0x4) == 0x4){
        ...
        GpioDataRegs.GPCDAT.bit.GPIO92 = 0;
        asm(" nop");
        asm(" nop");
        asm(" nop");
    }
