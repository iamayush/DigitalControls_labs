...
extern float matlabLockshadow; // matlabLockshadow
...
// MATLAB vars
#pragma DATA_SECTION(arrTime,".my_arrs")
float arrTime[1000];
#pragma DATA_SECTION(arrVel,".my_arrs")
float arrVel[1000];
#pragma DATA_SECTION(arrU,".my_arrs")
float arrU[1000];
#pragma DATA_SECTION(Amp,".my_arrs")
float Amp = 5;
...
void basicio(void)
{
    ...
    u = Amp*sin((u_time*0.001*2.0*PI)/1.0); // sine func
    theta=readEnc1();

    vel = (theta - theta_old)/SAMPLE_TIME; // rad/s
    ...
    if(time == 250)
    {
        UART_printfLine(1,"u=%.3f",u);
        UART_printfLine(2,"v=%.3f",vel);
        time =0;
    }

    if(u_time < (5*1000)){
        arrTime[(u_time/5)] = u_time;
        arrVel[(u_time/5)] = vel;
        arrU[(u_time/5)] = u;
    }

//    if (u_time >= 100000){
//        u_time = 0; // loop every 100s
//    }
    theta_old = theta; // saving old vals
    u_time += 5;
}
