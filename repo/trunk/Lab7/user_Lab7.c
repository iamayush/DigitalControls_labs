...
void mainCLKfunc(void){
    ...
    if (vel > 0.0) {
        u = u + 0.95*Vpos*vel + 0.95*Cpos;
    }
    else {
        u = u + 0.95*Vneg*vel + 0.95*Cneg;
    }
    ...
    if((timer%100) == 0){
        UART_printfLine(1, "i %0.2f v %0.2f",integral, vel);
    ...
    if (!SIMU_beginnewdata) {// Only true if have not yet begun a message
        if (SIMU_checkfirstcommandbyte == 1) {
            ...
            if (0x7F == (unsigned char)data) {// Check for start char
                ...
                SIMU_Var1_toSIMU_32bit = 10000*vel; //EQEP_readraw(&eqep1);
                SIMU_Var2_toSIMU_32bit = 10000*u; //EQEP_readraw(&eqep3);

                SIMU_Var1_toSIMU_16bit = 10*reference;
                SIMU_Var2_toSIMU_16bit = 0; //adcb1result;
