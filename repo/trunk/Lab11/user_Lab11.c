...
//Gains
float K[4] = {-8.5445,-3.3264,-94.3109,-12.4099};

// fric compensation
float Vpos = 0.008; //0.01266;
float Vneg = 0.008; //0.01246;
...
// Observer
float AD_LC[4][4]={{   -2.5026262542033217e-01,    5.0000000000000010e-03, 1.7259137854299750e-01, -1.0847366182939192e-06},
                   {   -7.7305163346228838e+01,    1.0000000000000000e+00, 2.0491088986224288e+01, -6.5088328683392065e-04},
                   {   1.7244090464269937e-01, 0.0000000000000000e+00, -4.2898206007574635e-01,    5.0015871740420248e-03},
                   {   2.0412161281657777e+01, 0.0000000000000000e+00, -9.8544108704406867e+01,    1.0009523648780978e+00}};

float Bd_L[4][3]={{   4.3018810489982762e-04, 1.2502626254203322e+00, -1.7324226182983141e-01},
                  {   1.7208248437959042e-01, 7.7305163346228838e+01, -2.0751483618314133e+01},
                  {   -1.6693774087949575e-04,    -1.7244090464269937e-01,    1.4299344249538442e+00},
                  {   -6.6785693376265748e-02,    -2.0412161281657777e+01,    9.8925115110722587e+01}};

float x_hat[4] = {0,0,0,0};
float x_hat0[4] = {0,0,0,0};
float u_old = 0;
float U[3] = {0,0,0};
float y1 = 0;
float y2 = 0;
...
void mainclkfunc(void){
    ...
    if (x3 > PI) {
        ...
        x3mod = x3 - 2.0*PI*ceil((x3-PI)/(2*PI));
    } else {
        x3mod = x3;
    }
    ...
    for (m = 0; m < 4; m++){
        x_hat[m] = 0;
        for (n = 0; n < 4; n++){
            x_hat[m] += AD_LC[m][n]*x_hat0[n];
        }
    }
    for (m = 0; m < 4; m++){
        for (n = 0; n < 3; n++){
            x_hat[m] += Bd_L[m][n]*U[n];
        }
    }

    u = -K[0]*x_hat[0] -K[1]*x_hat[1] -K[2]*x_hat[2] -K[3]*x_hat[3];

    //u = -K[0]*x1mod -K[1]*x2 -K[2]*x3mod -K[3]*x4;

    if (x2 > 0.0) {
        u = u + 0.95*Vpos*x_hat[1] + 0.95*Cpos;
    }
    else {
        u = u + 0.95*Vneg*x_hat[1] + 0.95*Cneg;
    }
    ...
    // print every 500 ms
    if((timer%100) == 0){
        UART_printfLine(1, "x1 %0.2f x3 %0.2f",ang1, x3mod);
        UART_printfLine(2, "u %0.2f", u);
    ...
    if (!SIMU_beginnewdata) {// Only true if have not yet begun a message
        if (SIMU_checkfirstcommandbyte == 1) {
            ...
            if (0x7F == (unsigned char)data) {// Check for start char
                ...
                SIMU_Var1_toSIMU_32bit = x4*10000;
                SIMU_Var2_toSIMU_32bit = x_hat[3]*10000;

                SIMU_Var1_toSIMU_16bit = setpoint*10;
